...
const StaffHome = (props: IProps) => {

    const [loading, setLoading] = useState(false);
    const [workloadSheets, setWorkloadSheets] = useState<IStaffResearch[]>([]);
    ...
    //----   COMPONENT DID MOUND   ----
    useEffect(() => {
        getStaffworkloadSheets();
    }, []);


    //----   GET STAFF workloadSheets   ----
    const getStaffworkloadSheets = async () => {
        setLoading(true);
        let qry = GET_ENDPOINT.GET_STAFF_WORKLOAD_SHEETS.toString();
    ...
    const add = async () => {
        ...
        let result = await Connection.postRequest(POST_ENDPOINT.ADD_STAFF_WORKLOAD_SHEET, dataToSend, {});
        setLoading(false);
        if (result.errorMessage.length > 0) {
            errorToast(result.errorMessage, true);
            return false;
    ...
    return (
        <>
            <div className="full-size">
                <TableComponent
                    title="Workload Sheets"
                    context={props.context}

                    ids={[...workloadSheets]}
                    headerValues={["Id", "Name", "", ""]}
                    data={
                        workloadSheets.map((r, index) => {
